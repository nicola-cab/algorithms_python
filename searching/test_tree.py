def test_tree(tree):
    ...
    print("Median is = ",tree_select(tree.root, 1))
    ...
    if tree.__str__() == "red black tree":
        print("Check if tree is rb tree ")
        print(is_23_tree(tree, tree.root))
...
def is_bst_tree(node):
    """ check if bst properties are respected """
...
def is_23_tree(tree,node): 
    """ check if the tree is red/black """
...
def is_23(tree,node):
    ...
    if tree.isRed(node.right):  
        return False
    if node != tree.root and tree.isRed(node) and tree.isRed(node.left):
        return False
    return is_23(tree, node.left) and is_23(tree, node.right)
